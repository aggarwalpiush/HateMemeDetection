...
class CLS4(Dataset):
    ...
    def __getitem__(self, index):
        ...
        emotion_tags = [box_sc['emotion'] for box_sc in idb['boxes_and_score']]
        ...
        if 'token_id' not in idb:
            ...
            person_tags_str = ''
            person_tags_part = []
            for j, (race, gend, emotion) in enumerate(zip(race_tags, gender_tags, emotion_tags)):
                if race is not None:
                    is_last = not any([rt is not None for rt in race_tags[j + 1:]])
                    append_str = f"{race.replace('_', ' ')} {gend} {emotion}"
        ...
        if self.use_img_box:
            if self.test_mode:
                ...
                label = torch.LongTensor([float(idb['label'])])
                return (
                    image, boxes, im_info, token_id, 
    ...
    def load_annotations(self):
        ...
        box_annos_json = os.path.join(self.root_path, 'box_annos.race_emotion.json')
        test_unseen_json = os.path.join(self.root_path, 'test_unseen.entity.jsonl')
        test_seen_json = os.path.join(self.root_path, 'test_seen.entity.jsonl')
        ...
        train1_json = os.path.join(self.root_path, 'train1.entity.jsonl')
        train2_json = os.path.join(self.root_path, 'train2.entity.jsonl')
        ...
        with open(train1_json, mode='r') as f:
            for line in f.readlines():
                train1_sample.append(json.loads(line))
        
        with open(train2_json, mode='r') as f:
            for line in f.readlines():
                train2_sample.append(json.loads(line))
        
        with open(dev_train_json, mode='r') as f:
            for line in f.readlines():
                dev_train_sample.append(json.loads(line))

        with open(test_unseen_json, mode='r') as f:
            for line in f.readlines():
                test_unseen_sample.append(json.loads(line))
        
        with open(test_seen_json, mode='r') as f:
            for line in f.readlines():
                test_seen_sample.append(json.loads(line))
        ...
        elif self.data_split == 'train1':
            sample_sets.append(train1_sample)
        elif self.data_split == 'train2':
            sample_sets.append(train2_sample)
        elif self.data_split == 'val':
            sample_sets.append(dev_sample)
        elif self.data_split == 'val_all':
            sample_sets.append(dev_train_sample)
        elif self.data_split == 'train+val':
            sample_sets.append(train_sample)
            sample_sets.append(dev_train_sample)
        elif self.data_split == 'test':
            sample_sets.append(test_seen_sample)
        elif self.data_split == 'test_all':
            sample_sets.append(test_unseen_sample)
            sample_sets.append(test_seen_sample)
        else:
