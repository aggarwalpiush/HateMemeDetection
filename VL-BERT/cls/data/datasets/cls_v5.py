...
class CLS5(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'token_id' not in idb:
            ...
            person_tags_str = ''
            person_tags_part = []
            for j, emotion in enumerate(emotion_tags):
                if emotion is not None:
                    is_last = not any([rt is not None for rt in emotion_tags[j + 1:]])
                    append_str = f"{emotion}"
    ...
    def load_annotations(self):
        ...
        box_annos_json = os.path.join(self.root_path, 'box_annos.emotion.json')
