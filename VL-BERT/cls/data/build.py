...
DATASET_CATALOGS = {'cls': CLS, 'cls2': CLS2, 'cls3': CLS3, 'cls4': CLS4, 'cls5': CLS5}
...
def make_data_sampler(dataset, shuffle, distributed, num_replicas, rank):
    if distributed:
        if shuffle:
            return samplers.DistributedWeightedSampler(dataset, shuffle=shuffle, num_replicas=num_replicas, rank=rank)
        else:
            return samplers.DistributedSampler(dataset, shuffle=shuffle, num_replicas=num_replicas, rank=rank)
    if shuffle:
        # sampler = torch.utils.data.sampler.RandomSampler(dataset)
        sampler = torch.utils.data.WeightedRandomSampler(
            dataset.weights_by_class, len(dataset), replacement=True)
    else:
...
def make_dataloader(cfg, dataset=None, mode='train', distributed=False, num_replicas=None, rank=None,
                    expose_sampler=False, norm_image=True, return_component=False):
    assert mode in ['train', 'val', 'test', 'train+val']
    if mode == 'train':
    ...
    transform = build_transforms(cfg, mode, norm_image=norm_image)

    if dataset is None:
        kwargs = {}
        if cfg.DATASET.ANNO_AUG:
            kwargs['anno_aug'] = True
        ...
        if cfg.DATASET.K_FOLD and mode == 'train':
            dataset = KFoldWrapper(dataset, k=cfg.DATASET.K_FOLD_K)
            dataset.set_fold(cfg.DATASET.K_FOLD_N)
    ...
    if return_component:
        return dataset, batch_sampler, collator
