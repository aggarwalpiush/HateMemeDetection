...
def test_net(args, config, ckpt_path=None, save_path=None, save_name=None):
    ...
    # for nbatch, batch in tqdm(enumerate(test_loader)):
        ...
        # print("label_logits", outputs['label_logits'])
        if outputs['label_logits'].shape[-1] == 1:
            # print('sigmoid')
            prob = torch.sigmoid(outputs['label_logits'][:, 0]).detach().cpu().tolist()
        else:
            # print('softmax')
            probas = torch.softmax(outputs['label_logits'], dim=-1).tolist()
            prob = torch.max(torch.softmax(outputs['label_logits'], dim=-1), dim=1).values
            label = torch.max(torch.softmax(outputs['label_logits'], dim=-1), dim=1).indices
            # prob = torch.softmax(outputs['label_logits'], dim=-1)[:, 1].detach().cpu().tolist()
        sample_ids = batch[-1].cpu().tolist()
        for pb, lab, id in zip(prob, label, sample_ids):
            predicts.append({
                'id': int(id),
                'proba': float(pb),
                'label': int(lab)
            })
        for proba, id in zip(probas, sample_ids):
            pred_pc = {"pc_empty": proba[0], "disability": proba[1], "nationality": proba[2], "race": proba[3], "religion": proba[4], "sex": proba[5]}
            predict_probas.append({
                'id': int(id),
                'set_name': config.DATASET.TEST_IMAGE_SET,
                'pred_pc': pred_pc
            })
