    ...
    def update(self, outputs):
        with torch.no_grad():
            ...
            if num_classes == 1:
                ...
                match = torch.argmax(cls_logits, dim=1) == label
                match = match.sum().item()
                print("output label_logits", outputs["label_logits"])
        ...
        if self.num_inst.item() == 0:
            ...
            if self.allreduce:
                ...
                for pred in self.predicts:
                    predicts.append(pred.clone().cuda()) # predicts is a list of tensors
                ...
                pred_list = [torch.FloatTensor([(-torch.ones_like(pred)).tolist() for pred in predicts]).cuda() for _ in range(size)] # pred_list is a list of tensors
                ...
                distributed.all_gather(pred_list, torch.FloatTensor([elem.tolist() for elem in predicts]).cuda())

                if distributed.get_rank() == 0:
                    for s, (l, p) in enumerate(zip(label_list, pred_list)):
                        _l = self.extract_and_filter(l)
                        _p = self.extract_and_filter(p)
                        # print(f"val subset - {s} roc_acu: {roc_auc_score(tf.keras.utils.to_categorical(_l), tf.keras.utils.to_categorical(_p), multi_class='ovo')}")
                        print(f"val subset - {s} roc_acu: {roc_auc_score(tf.keras.utils.to_categorical(_l, num_classes=6), _p, average='micro', multi_class='ovr')}")
                ...
                # mertric = roc_auc_score(tf.keras.utils.to_categorical(all_labels), tf.keras.utils.to_categorical(all_preds), multi_class='ovo')
                mertric = roc_auc_score(tf.keras.utils.to_categorical(all_labels, num_classes=6), all_preds, average='micro', multi_class='ovr')
                # metric_tensor = (sum_metric / num_inst).detach().cpu()
            else:
                # metric_tensor = (self.sum_metric / self.num_inst).detach().cpu()
                # mertric = roc_auc_score(tf.keras.utils.to_categorical(self.labels), tf.keras.utils.to_categorical(self.predicts), multi_class='ovo')
                mertric = roc_auc_score(self.labels, self.predicts, average='micro', multi_class='ovr')
